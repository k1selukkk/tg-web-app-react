...
const MoviesContainer = ({ searchQuery, filters }) => {
  ...
  useEffect(() => {
    if (searchQuery) {
      ...
      fetchMovies(`${API_URL_POPULAR}&genre=${filters.genre}`);
    } else {
  ...
  return (
    <div className="container">
      <div className="movies">
        {movies.map(movie => (
          <Movie key={movie.kinopoiskId} movie={movie} onClick={() => handleModal(movie.kinopoiskId)} />
        ))}
      </div>
